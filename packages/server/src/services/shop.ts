...
import { isRodSlug, isBaitSlug, getRod, getBait } from "@hooked/shared";
...
export async function buyRod(walletAddress: string, slug: string) {
  if (!isRodSlug(slug)) throw new ValidationError("Unknown rod");
  const rod = getRod(slug);

  // Atomic purchase: deduct shell and add rod iff player exists, can afford
  // it, and doesn't already own it.
  ...
  // Tier upgrade only if this rod outranks the equipped one; guard on rodTier
  // so a concurrent higher-tier purchase can't be stomped.
...
export async function buyBait(walletAddress: string, slug: string) {
  if (!isBaitSlug(slug)) throw new ValidationError("Unknown bait");
  const bait = getBait(slug);
...
export async function equipRod(walletAddress: string, slug: string) {
  if (!isRodSlug(slug)) throw new ValidationError("Unknown rod");
  const rod = getRod(slug);

  const result = await Player.findOneAndUpdate(
    { walletAddress, "equipment.ownedRods": rod.slug },
    {
      $set: {
        "equipment.rodEquipped": rod.slug,
        "equipment.rodTier": rod.tier,
  ...
  if (!result) {
    const existing = await Player.findOne({ walletAddress })
      .select({ _id: 1 })
      .lean();
    if (!existing) throw new NotFoundError("Player not found");
...
export async function equipBait(walletAddress: string, slug: string) {
  if (!isBaitSlug(slug)) throw new ValidationError("Unknown bait");
  const bait = getBait(slug);

  const result = await Player.findOneAndUpdate(
    { walletAddress, "equipment.ownedBaits": bait.slug },
    {
      $set: {
        "equipment.baitEquipped": bait.slug,
        "equipment.luckyLureTier": bait.tier,
...
export async function sellFish(walletAddress: string, catchId: string) {
  if (!mongoose.isValidObjectId(catchId)) {
  ...
  const claimed = await Catch.findOneAndUpdate(
    { _id: catchId, playerId: player._id, released: false },
    { $set: { released: true, soldAt: now } },
    { new: true, projection: { sellValue: 1, rarity: 1 } },
  ).lean();

  if (!claimed) throw new ValidationError("Catch not available");
  ...
  return { price, shellBalance: updatedPlayer?.shellBalance ?? 0 };
}

export async function sellFishBulk(walletAddress: string, catchIds: string[]) {
  const validIds = catchIds.filter((id) => mongoose.isValidObjectId(id));
  if (validIds.length === 0) throw new ValidationError("No valid catch ids");

  const player = await Player.findOne({ walletAddress }, { _id: 1 }).lean();
  if (!player) throw new NotFoundError("Player not found");

  const candidates = await Catch.find(
    { _id: { $in: validIds }, playerId: player._id, released: false },
    { _id: 1, sellValue: 1 },
  ).lean();

  if (candidates.length === 0) throw new ValidationError("Nothing to sell");

  // Claim each catch atomically and in parallel; only catches we flipped to
  // released count, guarding against concurrent double-sells.
  const now = new Date();
  const outcomes = await Promise.all(
    candidates.map(async (c) => {
      ...
      return res.modifiedCount === 1 ? { id: String(c._id), price } : null;
    }),
  );
  ...
  if (sold.length === 0) throw new ValidationError("Nothing to sell");
